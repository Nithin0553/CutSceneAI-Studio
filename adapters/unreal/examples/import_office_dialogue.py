"""Generated by CutSceneAI Unreal Adapter v0.2 for Unreal Engine 5.8.0.
...
def _configure_static_mesh(actor, asset_path):
...
def _add_set_piece(sequence, subsystem, set_piece):
    ...
    if template is None:
        raise RuntimeError(f"Set piece has no object template: {set_piece['binding_id']}")
    template.set_actor_label(set_piece["display_name"])
    _set_transform(template, set_piece["transform"])
    _configure_static_mesh(template, set_piece["mesh_asset_path"])
...
def _add_actor(sequence, subsystem, actor):
    ...
    _set_transform(template, visual["transform"] if visual else actor["transform"])

    asset_path = visual["mesh_asset_path"] if visual else actor["asset_path"]
    if asset_path is not None:
        _configure_static_mesh(template, asset_path)
    subsystem.save_default_spawnable_state(binding)
    return binding
