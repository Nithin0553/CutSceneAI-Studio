"""Generated by CutSceneAI Unreal Adapter v0.1 for Unreal Engine 5.6.
...
def _add_camera(sequence, subsystem, camera):
    ...
    template = binding.get_object_template()
    if template is None:
        raise RuntimeError(f"Camera has no object template: {camera['binding_id']}")
    template.set_actor_label(camera["display_name"])
    _set_transform(template, camera["transform"])
    template.get_cine_camera_component().set_editor_property(
        "current_focal_length", float(camera["lens_mm"])
    )
    return binding
