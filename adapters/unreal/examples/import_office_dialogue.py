...
def _package_root():
    ...
    if not script_file:
        raise RuntimeError(
            "The v0.7 importer must run from its extracted .py file so bundled WAV paths "
...
def _preflight_import():
    ...
    imported_audio_targets = {
        item["asset_path"] for item in PLAN["audio_imports"]
    }
    referenced_assets = set()
    for scene in PLAN["sequences"]:
        referenced_assets.update(
            item["mesh_asset_path"] for item in scene["set_pieces"]
        )
        ...
        referenced_assets.update(
            item["asset_path"] for item in scene["animation_sections"]
        )
        referenced_assets.update(
            item["asset_path"] for item in scene["audio_sections"]
        )

    missing = sorted(
        asset_path
        for asset_path in referenced_assets
        if asset_path not in imported_audio_targets
        and not unreal.EditorAssetLibrary.does_asset_exist(asset_path)
    )
    if missing:
        formatted = "\n".join(f"- {asset_path}" for asset_path in missing)
        raise RuntimeError(
            "Import plan references missing Unreal assets. Refresh the project index or "
            f"repair explicit CIR paths before retrying:\n{formatted}"
        )
