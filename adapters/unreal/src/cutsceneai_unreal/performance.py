...
DEFAULT_GENERATED_PERFORMANCE_PATH = "/Game/CutSceneAI/GeneratedPerformance"
...
def _validate_export_plan(
    ...
    export_plan: UnrealExportPlan,
    semantics: TimelineSemantics,
) -> str:
    scene = verify_performance_bundle_semantics(bundle, semantics)
    if (
        export_plan.project_id != bundle.package.project_id
        or export_plan.source_settings.fps != bundle.package.fps
        or len(export_plan.sequences) != 1
    ):
        raise ValueError(
            "Unreal export plan identity or frame rate does not match the performance bundle."
        )
    ...
    if (
        ...
        raise ValueError(
            "Unreal export plan scene does not match performance timeline semantics."
        )
    ...
    if actual_actors != expected_actors:
        raise ValueError(
            "Unreal actor bindings do not exactly match performance timeline semantics."
        )
    cut_by_id = {item.cut_id: item for item in scene.camera_cuts}
    expected_cameras = sorted(
        (
            track.camera_binding_id,
            cut_by_id[track.source_camera_cut_id].source_shot_id,
            track.start_frame,
            track.end_frame,
        )
    ...
    actual_cameras = sorted(
        ...
        for item in sequence.cameras
    )
    if actual_cameras != expected_cameras:
        raise ValueError(
            "Unreal camera bindings do not exactly match generated camera tracks."
        )
...
def compile_performance_bundle(
    ...
    *,
    ...
    """Convert one verified package into a deterministic Unreal 5.8 mapping plan."""

    if not re.fullmatch(r"/Game(?:/[A-Za-z0-9_]+)+", target_path):
        raise ValueError("target_path must be a normalized Unreal /Game package path.")
    source_scene_id = _validate_export_plan(bundle, export_plan, semantics)
        ...
        body_tracks.append(
            UnrealGeneratedBodyTrack(
                ...
                target_animation_path=(
                    f"{target_path}/Body/AN_{_asset_name(body_track.semantic_id)}"
                ),
                ...
                keyframes=[
                    UnrealBodyKeyframe(
                        ...
                        root_location_cm=convert_position(sample.root_translation),
                        joint_rotations=[
    ...
    for facial_track in package.facial_tracks:
        ...
        facial_tracks.append(
            UnrealGeneratedFacialTrack(
                ...
                target_animation_path=(
                    f"{target_path}/Face/FA_{_asset_name(facial_track.semantic_id)}"
                ),
            ...
            UnrealGeneratedCameraTrack(
                ...
                keyframes=[
                    UnrealCameraKeyframe(
                        ...
                        location_cm=convert_position(sample.position),
    ...
    audio_tracks = [
        UnrealGeneratedAudioTrack(
            ...
            target_sound_path=(
                f"{target_path}/Audio/SW_{_asset_name(track.dialogue_cue_id)}"
            ),
        )
        for track in package.audio_tracks
    ]
    return UnrealPerformanceMapping(
