...
- one `.playable` Timeline and one `.unity` Scene per CIR scene;
...
- semantic markers and an attached semantic manifest carrying the unchanged CIR fingerprint.
...
complete preflight and refuses to replace an existing generated Scene or Timeline.

## Run in Unity

1. Use Unity 6000.0 with `com.unity.timeline` 1.8.12.
2. Copy `CutSceneAIGeneratedTimeline.cs` into the Unity project's `Assets/Editor/` folder.
